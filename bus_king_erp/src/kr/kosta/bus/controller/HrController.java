...
//		int totalPage = total / rowSize + (total % rowSiez == 0 ? 0 : 1);
	...
	public  String pay_insert(HttpServletRequest request, Model model) {
		if(		request.getParameter("pay_e_no").length() < 1 || request.getParameter("pay_e_name").length() < 1 ||
				request.getParameter("pay_e_dname").length() < 1 || request.getParameter("pay_e_position").length() < 1 ||
				request.getParameter("pay_year").length() < 1 || request.getParameter("pay_month").length() < 1 ||
				request.getParameter("pay_day").length() < 1 || request.getParameter("pay_chul").length() < 1 ||
				request.getParameter("pay_work").length() < 1 || request.getParameter("pay_work_t").length() < 1 ||
				request.getParameter("pay_p_t").length() < 1 || request.getParameter("pay_p").length() < 1 ||
				request.getParameter("pay_m").length() < 1 || request.getParameter("pay_tax").length() < 1 ||
				request.getParameter("pay_nametag").length() < 1) {
